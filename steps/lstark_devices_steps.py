...
@step('ls Verify I on "{page_name}" page')
def ls_verify_on_page(context, page_name):
...
@step('ls I open Devices Settings')
def ls_open_devices_settings(context):
...
@step('ls Press Add new device button')
def ls_press_add_new_device(context):
...
@step('ls Choose "{device_type}" device type')
def ls_choose_device_type(context, device_type):
...
@step('ls Fill out name "{device_name}" of device')
def ls_fill_device_name(context, device_name):
...
@step('ls Press add new device button')
def ls_press_add_device_button(context):
...
@step('ls Verify device "{device_name}" exists in list of devices')
def ls_verify_device_exists(context, device_name):
...
@step('ls Go to "{page_name}" page')
def ls_open_menu(context, page_name):
