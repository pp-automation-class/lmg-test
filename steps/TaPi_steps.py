...
def should_be_on_devices_page(context):
    ...
    context.page.wait_for_selector(devices_page.page_title, state="visible", timeout=15000)
...
def should_remain_on_login_page(context):
    ...
    assert login_page.is_element_visible(login_page.email_input), "Expected to remain on Login page, but login elements are not visible."
...
def should_see_error_message(context, text):
    ...
    # Wait up to 10s for either the specific error element or the text to appear
    try:
        context.page.wait_for_selector(f"text={text}", timeout=10000, state="visible")
    except Exception:
        # If the text didn’t appear via text engine, try the page object’s error locator
        try:
            context.page.wait_for_selector(login_page.error_message, timeout=10000, state="visible")
        except Exception:
            pass

    actual = login_page.get_error_message()
    if actual:
        assert text.lower() in actual.lower(), f"Expected error message to contain '{text}', got '{actual}'"
    else:
        is_visible = context.page.is_visible(f"text={text}")
        assert is_visible, f"Expected to see error message '{text}', but it was not visible."