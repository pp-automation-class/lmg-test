...
class TaPiDevicesPage(BasePage):
    """Represents the Devices page for the TaPi brand with its elements
    and actions."""
    ...
    def __init__(self, page):
        ...
        # Devices page header/title. Use normalize-space to tolerate
        # extra spaces.
    ...
    def get_device_locator(self, name: str):
        """Return the locator for a specific device card by its
        display name."""
        # Match the device name exactly inside the device info header
        return (f"//div[@class='lmg-device__info']"
                f"/h4[normalize-space(text())='{name}']")
